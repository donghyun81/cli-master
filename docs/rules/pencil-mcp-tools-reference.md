...
- 2026-05-19 · MASTER-CLI-PENCIL-OPTIMIZATION-002 · §1.1 `batch_design` 하위 **3 신설** (§1.1.1 `.pen` 13 Entity type 정합 호출 / §1.1.2 Component·Instance·Slot 호출 / §1.1.3 Variable substitution) + §1.2 `batch_get` 하위 **3 신설** (§1.2.1 13 Entity type 정합 검색 / §1.2.2 Nested component search / §1.2.3 Component instance audit) = **절 6 신설** (자 = `git show --numstat 08104e9` → **67 추가 / 0 삭제** · 날짜 자 = `git log -1 --format=%ad --date=short 08104e9`) + 5-repo byte-identical propagation. **소급 등재** (= 2026-08-30 `MASTER-DOC-MANIFEST-SWEEP-002`). ★**귀속은 낱말이 아니라 배선이 판정한다** — 신설된 헤딩의 문면이 「강화」여서 **인용으로 오독**됐으나(= `rule-footer-common.md:10` 「인용만 하는 헤더는 대상 아님」의 오적용), 그 헤딩을 **만든 것이 이 cycle**이고 헤딩이 **자기 ID 를 자칭**한다 ⟹ 귀속. ★**누락의 기전 = 자의 깊이 맹점** — 등재 판정에 쓰이던 `^+#{1,3} ` 는 `####`(깊이 4)를 **구조적으로 못 본다**(실측 = 같은 commit 이 깊이 4 로 **6** 을 신설했는데 그 자는 **0**). 정정된 자 = `^+#+ ` (= 깊이 무관 · `verification-and-review.md` §0.6 정합).
