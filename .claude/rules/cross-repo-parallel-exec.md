...
> **H4 demote** (`MASTER-CLI-CONTEXT-OPT-PHASE3-L0-CHILD-DEDUP-001` · 2026-06-01): cross-repo 실행 paradigm **본문**(영역 1/2/3 + dispatch + 경계 + 자식별 cwd 분리 + sub-agent token cost + 정합 처리)은 behavior-triggered 로 demote → [`cross-repo-parallel-exec-detail.md`](./cross-repo-parallel-exec-detail.md) (= cross-repo 행동 진입 시 정독 · `rule-routing-index.md §B` Reading Mode 6). 본 kernel = 항상-on L0 (= 아래 §2.4 subscription/billing guard + 단방향 propagation + 영역 1/2/3 1-줄 요약). 선택 본심 = cli session 자율 (= 요청 본질 측정 후 영역 1 vs 영역 2).
