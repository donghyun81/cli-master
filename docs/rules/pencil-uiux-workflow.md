...
자식 `.pen` 이 foundation Neutral 색을 capture 한 gray-trap 을 활성 자식 brand colorScheme 으로 결정론 remap 하는 경로. 본문 단일 SoT = [`.claude/skills/pencil-recolor/SKILL.md`](../../.claude/skills/pencil-recolor/SKILL.md). `.pen` = plain JSON (`version: "2.11"`) 직접 read/write — Pencil app / MCP tool 불요 (= `pen_recolor.py` 측 `json.load`).
