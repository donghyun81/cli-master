...
> **단일 목적**: 4-repo cli infra 의 **측정 가능 목표**를 표현하는 canonical 3-tuple form(Goal–Signal–Metric) + 측정 layer(anchor / 행동 / context-health / cycle-health) 의 단일 기준. 흩어진 측정 인접 자산(anchor-list A1~A10 · `rule-routing-index §C` 행동 7종 · `context-health-metrics`)이 본 form 에 **정합**한다.
...
> - [`anchor-list.md`](../../.claude/rules/anchor-list.md) — anchor A1~A10 의 G/S/M 3-tuple
...
|---|---|---|---|
| **anchor** | 누락 시 cycle 실패하는 10 anchor(A1~A10) | `anchor-list.md` 각 anchor 의 G/S/M | 4-repo byte-identical |
| **행동(Reading Mode)** | 7종 행동별 목표·deviation | `rule-routing-index.md §C` | 4-repo byte-identical |
...
| 구성 | 역할 | 위치 | 전파 |
|---|---|---|---|
| `measure-gsm-cycle.sh` | Stop hook · git log 에서 DORA proxy 산출 + surface(advisory) | `.claude/hooks/` | 4-repo byte-identical |
