...
> **§15 cold 재배치** (= **27 회차 누적** · `MASTER-CLI-CONTEXT-OPT-PHASE1-CYCLE-HISTORY-COLD-001` 2026-06-01 → … → **16 회차** `MASTER-BRAND-TOWARD-INFRA-001` 2026-08-15 → **무번호 8**(2026-08-17~08-29) → `MASTER-MEASURE-DISCIPLINE-001` 2026-08-30(`b3a8857` · **실서수 25** · 구 라벨 ~~17~~) → `MASTER-DOC-MANIFEST-SWEEP-003`(`a3ba108` · **실서수 26** · 구 라벨 ~~18~~) → **본 판 27 회차** · 회차 전량 열거 = COLD §1 heading lineage): master cycle 전체 이력 (= `C1-MASTER-BOOTSTRAP-001` ~ 본 cycle 직전) = verbatim 보존 → [`.auto-memory/master-cycle-history-COLD.md`](.auto-memory/master-cycle-history-COLD.md) (= 현행 **168 데이터행 / 166 고유 cycle ID** · 중복 2 · 자 = `grep -c '^| [A-Z0-9]'` / `cut -d'|' -f2` 후 `sort -u` · 2026-08-30 실측(SWEEP-004 demote +1) · 구 표기 = 167 · 그 전 = 165 · 그 전 = 149 · 삭제 0 · 감사 추적 영구 보존 · lifecycle = 매 5 cycle 또는 분기 review). **hot 압축 행의 원문도 COLD 에 verbatim 실재** (= 재작성 ≠ 소실).
