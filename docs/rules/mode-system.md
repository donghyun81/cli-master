...
> 본 file = 4-repo 측 Mode 시스템 영구 SoT default · Mode = AI 행동 layer default · Phase A/B/C/D/F = 작업 lifecycle 위치 layer default
> 위치 = `claude-cli-master/docs/rules/mode-system.md`
...
- **Purpose**: cli infra (= `.claude/` + `docs/schemas/` + 보호 file + `scripts/` default) 변경 + 4-repo byte-identical propagation default — production code 무접촉 + 단방향 propagation 정합 default
- **Method defaults**: master 측 단방향 source default + scripts/propagate.sh + scripts/verify-sync.sh + 4-repo cross-verify + propagation-reports/<cycle-id>/REPORT.md 자동 생성 + production code touch 0 LOC 의무 default
...
## §4. 현 단계 = 4-repo 동일 mode / 미래 = 자식별 발산 protocol (= L1-6 정합 default)

- 현 4-repo 측 default mode = M1 (= production-graduated default)
