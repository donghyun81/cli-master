...
#     (1) 현재 4-active HEAD + 보호 5 file sha 를 live 측정해 AI-readable 1-블록으로 표면화하고
...
# mount root (= repo 들이 직접 놓인 디렉터리) robust 탐지 (baseline-snapshot.sh 와 동일 분기).
#   §3.1 자식 단독 진입 → repo = dirname(PROJECT_DIR) 하위
#   §3.2 부모 mount 진입 → repo = PROJECT_DIR 하위
...
# live 측정(4-active HEAD + 보호 5 sha) + baseline drift 비교 + additionalContext emit.
