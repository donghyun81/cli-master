...
| **cross-version 버전업 마이그 (= 2.11→2.13 · §7.3)** | **delta-aware 변환 + pre-scan(전수) + post-check(grep=0)** (= §1.1a delta 전수 · `save()` 재직렬화 금지 · 0 byte risk · inconsistent 2.13 차단) |
...
## 부록 (= §7.3 멀티-repo umbrella caveat (= 마이그레이션 백과) · §12 cycle 이력)

> 전문 = [`references/multirepo-caveat-and-history.md`](references/multirepo-caveat-and-history.md) verbatim (= 2026-07-29 분할 · 판정에 매번 필요하지 않은 층 · 삭제 0).
## 8. Model 선택 paradigm
