...
> 본 file = 6-repo 측 plugin(= Cowork / Claude Code plugins + marketplaces) 정책 영구 단일 SoT default · 판단 기준 유일 본문 default
...
- **커스텀 불필요 + 공식·공신력** = official marketplace(Anthropic 큐레이션) 또는 검증 통과 community plugin이면서, 6-repo 전부 동일 구성으로 충분한 영역 → 허용.
...
- 기준 변경 시 = 본 file 단일 변경 + 6-repo byte-identical propagation default (= 자식 측 본문 보유 X이므로 pointer 무변경).
...
2. **byte-identical 정합 측정**(§6) = 후보 plugin이 6-repo 동일 구성으로 충분한지 판정.
...
## §6. propagation 정합 (= 6-repo byte-identical 보존 default)

- cli infra(= `.claude/`)는 master 단방향 propagation + 6-repo byte-identical default (= master `CLAUDE.md §3` + `cross-repo-parallel-exec.md §4.3`).
