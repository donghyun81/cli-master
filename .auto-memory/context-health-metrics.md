...
| master CLAUDE.md (FULL) | 43,062 (2026-07-17 재측정 · `MASTER-CLI-S15-HOT-DEMOTE-005` 9회차 demote 후 · **advisory 무발화 = hot 7 ≤ 10**) | §15 = hot 7 entry + cold pointer(전체 이력 = `master-cycle-history-COLD.md` 137 entry) · 직전 47,004 (07-10 hot 13 = advisory 발화 상태) — 감소분 = 9회차 cold 재이전 16 entry(hot 22→7) · 순감 ~3.9K (= demote 감소분 − 본 cycle 신규 2 entry[T7 + demote] + §1 registry 재저작 증가분 상쇄 후) |
...
| `master §15 hot entry` | 7 | **9회차** cold 재이전(MASTER-CLI-S15-HOT-DEMOTE-005 · 2026-07-17 · hot 22→5+본 cycle 계열 2[T7 + 본 demote] = **7** · demote 16 · cold 121→137 · 무손실 대칭 sha `0301724f` · 직전 8회차 = S15-HOT-DEMOTE-004 2026-06-22 hot 15→6) 후 · ≥ ~10 도달 시 cold 재이전 trigger — **재증식 자동 감시** = `measure-gsm-cycle.sh` §15 hot check(> 10 시 Stop hook advisory surface · warn-only · 이전 판정 = 수동) |
...
- 2026-07-17 · MASTER-CLI-S15-HOT-DEMOTE-005 · §15 hot 9회차 cold 재이전 (= hot 22 → 7 · demote 16 entry `MASTER-SUPABASE-PROD-APPLY-RECIPE-001`~`MASTER-CLI-SOTMAP-REGISTER-001` → `master-cycle-history-COLD.md` verbatim append · cold 121→137 · **무손실 대칭 실측 = removed 16 == added 16 · block sha-256 `0301724f0a0bdb46…` 일치** · 잔존 hot = 최근 5 + 본 cycle 계열 2[`MASTER-T7-INSTRUCTIONS-REALIGN-001` + 본 entry] = **7** · DEMOTE-004 8회차 동형). §2 재측정: master CLAUDE.md 47,004→**43,062** cp (= advisory 무발화 상태 복귀 · hot 7 ≤ 10 · `measure-gsm-cycle.sh` S15 hot check 재실행 무발화 확인) · `master §15 hot entry` 행 = 9회차 desc 갱신. 동반 정정: T7 §15 entry append 시 유입된 표 split 빈 줄 1(= T6 row ↔ T7 row 사이) 제거 → §15 표 연속 6행 복구(DEMOTE-004 "표 split 빈 줄 0" 계약 정합). master-only(propagation X) · 보호 5 sha drift 0 · production 0 LOC.
