...
> **§15 cold 재배치** (= **29 회차 누적**(= 구 표기 ~~28~~ · ★**머리·꼬리 동시 정정** = 본 판 · 직전 판이 남긴 「꼬리만 고치고 머리를 남기면 한 줄이 자기와 갈린다」[= `SWEEP-004 REPORT:172` T5-⑴] 를 **본 판은 같은 edit 안에서** 처리했다) · `MASTER-CLI-CONTEXT-OPT-PHASE1-CYCLE-HISTORY-COLD-001` 2026-06-01 → … → **16 회차** `MASTER-BRAND-TOWARD-INFRA-001` 2026-08-15 → ★**소급 채번 8 = 실서수 17~24**(2026-08-17~08-29 · 구 표기 ~~무번호 8~~ · 본 판 T4 마감 · 표 = COLD `§1` 채번 note) → `MASTER-MEASURE-DISCIPLINE-001` 2026-08-30(`b3a8857` · **실서수 25** · 구 라벨 ~~17~~) → `MASTER-DOC-MANIFEST-SWEEP-003`(`a3ba108` · **실서수 26** · 구 라벨 ~~18~~) → `MASTER-DOC-MANIFEST-SWEEP-004`(`814f591` · **실서수 27**) → `MASTER-REGISTER-AXIS-REPLACE-001`(`bc04cac` · **실서수 28**) → **본 판 29 회차**(= 구 표기 ~~28~~ 은 직전 판까지의 누적 · ★**라벨 인용 아님** = 사건 계수 재도출 + 산술 검산 `append 합 169 == 실측 169`) · 회차 전량 열거 = COLD §1 heading lineage): master cycle 전체 이력 (= `C1-MASTER-BOOTSTRAP-001` ~ 본 cycle 직전) = verbatim 보존 → [`.auto-memory/master-cycle-history-COLD.md`](.auto-memory/master-cycle-history-COLD.md) (= 현행 **170 데이터행 / 168 고유 cycle ID** · 중복 2 · 자 = `grep -c '^| [A-Z0-9]'` / `cut -d'|' -f2` 후 `sort -u` · 2026-08-30 실측(RULE-HISTORY-SECTION demote +1 · 그 전 REGISTER-AXIS-REPLACE demote +1 · 그 전 SWEEP-004 demote +1) · 구 표기 = 169 · 그 전 = 168 · 그 전 = 167 · 그 전 = 165 · 그 전 = 149 · 삭제 0 · 감사 추적 영구 보존 · lifecycle = 매 5 cycle 또는 분기 review). **hot 압축 행의 원문도 COLD 에 verbatim 실재** (= 재작성 ≠ 소실).
