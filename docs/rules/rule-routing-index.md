...
> **단일 목적**: `docs/rules/` 41 + `.claude/rules/` 6 = **47 rule**(= 색인 자신 제외 · 실측 자 = `ls -1 docs/rules/*.md | wc -l` + `ls -1 .claude/rules/*.md | wc -l`) 의 논리적 계층(L0~L3) 외화 + Reading Mode 7종(행동) → 의무 로드 규칙 집합 매핑 + 행동별 측정 가능 목표(GSM) + deviation 경로 + 색인 갱신 규약. **물리 폴더 이동 없음 · frontmatter 없음** — 본 색인 한 file 이 계층을 표현한다.
...
rule 은 **두 평면 폴더**에 산다(`docs/rules/` = 콜드 · `.claude/rules/` = 자동 주입 · 각 subfolder 0). 본 색인은 자신을 제외한 **47 rule** 을 그 평면 위에 **논리적 4층**으로 덮어 행동별로 무엇을 먼저 읽어야 하는지 가리킨다.
...
사용 흐름: **(1)** Reading Mode 판정(`workflow-core.md` §Intake) → **(2)** [`rule-routing-table.md`](../../.claude/rules/rule-routing-table.md) 표에서 해당 행동의 의무 로드 집합 추출 → **(3)** L0 + 표가 가리키는 L1/L2/L3 subset 만 연다. bulk read 금지(`CLAUDE.md §9`).
...
| [`rule-footer-common.md`](../../.claude/rules/rule-footer-common.md) | rule 공통 "변경 정책" footer canonical (= 각 rule 꼬리 1줄 pointer 의 본문 · T6) |
...
> **본문 = [`rule-routing-table.md`](../../.claude/rules/rule-routing-table.md)** (= MASTER-CLI-CONTEXT-DIET-2-001 T2 · **intake 시 그 표만 정독** · L0 정의 + T5 L0 재정독 개정 + T4 abbreviation-policy 의무 로드 제외 반영). 개정 전 §B 원문 = [`.auto-memory/rule-routing-index-COLD.md`](../../.auto-memory/rule-routing-index-COLD.md) (master-only). 표 갱신 = §D 와 동기 의무.
...
5. **(선택) verify hook pointer**: 본 색인 §A 멤버 수 + 색인 자신 1 = `ls -1 docs/rules/ | wc -l` + `ls -1 .claude/rules/ | wc -l` **두 경로 합산**(= repo root 기준 · 현재 42 + 6 = **48**) 정합 검증을 분기 review(`cycle-discipline §18`) 측 측정 항목으로 둘 수 있다(자동 hook 신설은 별 cycle).
...
- 본 file = cli infra **권장 byte-identical** 영역(실 위치 = `docs/rules/` · 보호 5종 아님 · `cycle-discipline §3`). 41 rule 이 4-repo byte-identical 이므로 본 색인도 4-repo 에서 동일하게 유효 → `propagate.sh` 대상.
...
| 4 | /verify · /review 규칙 | `docs/rules/verification-and-review.md` | `workflow-core.md` §/verify·§/review (hub 1줄 + pointer) |
| 5 | 코드 리뷰 12-section 체크리스트 | `docs/rules/verification-and-review.md` §"12-section 체크리스트" | `reviewer.md` Expected outputs (→ pointer + 역할 고유 보존) · `code-principles.md §4` (기존 pointer 유지) |
| 6 | SoftBudget (Risk별 LOC budget 200/120/60) + Risk 기반 산출물 경량화 | `docs/rules/workflow-core.md §implement` | `mode-system.md` M1 Verification policy + §7 (→ pointer · 이전 `code-principles.md §SoftBudget` = 부재 § stale pointer 정정 · MASTER-CLI-CONTEXT-OPT-PHASE4-SSOT-SWEEP-METRICS-001) |
...
| 8 | UI 라이브러리 억제 / 직접 구현 우선 | `docs/rules/ui-ux-analysis.md §UI 라이브러리 억제 기본값` | `DEPENDENCY_DECISION_CHECKLIST.md §3` (→ pointer · ⑧ 게이트 항만 유지) · `code-principles.md §UI 라이브러리 억제 default` (→ pointer) · master `CLAUDE.md §10` (→ pointer) |
