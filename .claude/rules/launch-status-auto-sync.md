# LAUNCH-STATUS + INDEX + Task file Auto-Sync Paradigm SoT

> **단일 목적**: 자식 repo (= GB / GD / GT) 측 cycle REVIEW PASS 시점 cli session 측 `docs/release-readiness/LAUNCH-STATUS.md` + `.ai/tasks/INDEX.md` + `.ai/tasks/<CYCLE-ID>.md` 3 file 갱신 의무 paradigm 영구 SoT. trigger 기반 paradigm default (= 자식 cycle 마감 시점 진입 default · paste-back 본문 발행 직전 시점 default · 매 cycle 마감 시점 catch-up drift 회피 default).
> **신설**: MASTER-CLI-LAUNCH-STATUS-AUTO-SYNC-PARADIGM-001 (2026-05-22).
> **precedent**: `MASTER-CLI-RUNTIME-CRASH-MITIGATION-PROCESS-PARADIGM-001` (= H32 마감 default · self-contained 11 section format default · 본 SoT format 차용 default).
...
> - `cycle-discipline.md` §15 패턴 1 (master 측 cli infra 단방향 propagation default)
> - `cycle-discipline.md` §20 (DocSync 단계 본문 default · 자식 출시 docs 영역 명시 default)
> - `cycle-discipline.md` §23 (Recommended option disk verification paradigm default)
> - `cycle-discipline.md` §24 (Runtime crash mitigation process paradigm default)
> - `cycle-discipline.md` §25 (= 본 SoT 측 pointer default)
> - `recommended-option-disk-verification.md` (disk 측정 의무 paradigm default · 본 paradigm 측 baseline 측정 영역 정합 default)
> - `workflow-core.md` §단계 흐름 (DocSync bullet default · 본 paradigm 측 trigger 시점 정합 default)
> SOT: `CLAUDE.md`
...
본 paradigm 본질 = self-consistent default · 본 paradigm 신설 cycle 측 paste-back 자체 = 본 paradigm 측 사례 default. 본 SoT 신설 cycle (= MASTER-CLI-LAUNCH-STATUS-AUTO-SYNC-PARADIGM-001) 측 paste-back `[EC]` 섹션 측 본 paradigm 본질 사례 명시 default.

단 본 SoT 신설 cycle = cli infra rule 영역 default · master 측 LAUNCH-STATUS 영역 부재 default + `.ai/tasks/` 영역 부재 default (= 자식 도메인 영역 default) → 본 SoT 신설 cycle 자체 측 3 의무 영역 진입 X default (= 자식 도메인 cycle 측 본 paradigm 본질 사례 default).
...
| §0.3 사고 영역 disk 측정 | LAUNCH-STATUS HEAD drift + INDEX 갱신 drift + 신 task file 누락 영역 disk 측정 결과 인용 default (= `recommended-option-disk-verification.md` §3 정합) |
...
| 인접 entry | 본질 |
|---|---|
| `cycle-discipline.md` §5 v2 line 75 | [agent-commit: yes] 묵시 동의 paradigm default · 본 paradigm 측 commit step 정합 default |
| `cycle-discipline.md` §7 | commit body 6-section 표준 default · 본 paradigm 측 §9 정합 default |
| `cycle-discipline.md` §15 패턴 1 | master 측 cli infra 단방향 propagation default · 본 SoT 측 propagation 영역 정합 default |
| `cycle-discipline.md` §20 | DocSync 단계 본문 default · 자식 출시 docs 영역 명시 default · 본 paradigm 측 LAUNCH-STATUS 갱신 의무 본문 정합 default |
| `cycle-discipline.md` §23 | Recommended option disk verification paradigm default · 본 paradigm 측 baseline 측정 영역 정합 default |
| `cycle-discipline.md` §24 | Runtime crash mitigation process paradigm default · 본 paradigm 측 동족 trigger 기반 paradigm 정합 default |
| `recommended-option-disk-verification.md` | disk 측정 의무 paradigm default · 본 paradigm 측 LAUNCH-STATUS HEAD drift 측정 + INDEX 갱신 drift 측정 + 신 task file 누락 영역 측정 default 정합 |
| `workflow-core.md` §단계 흐름 | DocSync bullet default · 본 paradigm 측 trigger 시점 정합 default |
...
- 2026-05-22 · `MASTER-CLI-LAUNCH-STATUS-AUTO-SYNC-PARADIGM-001` · 본 SoT 신설 (= paradigm 본질 + trigger 조건 + 3 의무 영역 + verify 의무 본문 + cli session 자율 paradigm + STOP 조건 + 적용 영역 + paste source authoring 영역 + commit body 본문 + 인접 paradigm 정합 default) + `cycle-discipline.md` §25 pointer 추가 default + CLAUDE.md §15 entry append default + 5-repo byte-identical propagation default
- precedent: `MASTER-CLI-RUNTIME-CRASH-MITIGATION-PROCESS-PARADIGM-001` (2026-05-22 H32 마감 default · self-contained 11 section format default · 본 SoT format 차용 default)
