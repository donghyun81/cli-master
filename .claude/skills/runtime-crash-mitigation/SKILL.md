...
**canonical 9 항 = [`.claude/rules/stop-canonical.md`](../../rules/stop-canonical.md)** (= 자동 주입 · 여기서 재복제하지 않는다). 아래 = 본 skill **고유** trigger 만:
...
| production data INSERT / DELETE / UPDATE 영역 root cause 발견 | 즉시 STOP + 사용자 회수 default (= canonical #1 의 crash-mitigation 발현형 · 명시 보존) |

구 판이 여기 재복제해 두었던 4 항 = canonical 로 이미 덮인다: 보호 sha drift(**#5**) · DB schema/migration root cause(**#1**) · Auth/Billing root cause 시 production 진입(**#1**·**#7**) · 사용자 본심 분기(**#9**).
...
## 부록 (= §7 paste 구조 + §8 commit body + §9 인접 paradigm · §11 cycle 이력)
