...
3. **구글식 엔지니어링 관행을 기준선으로 둔다** — 문서화 · 주석 · 코드 관리 · **의존성** · 유지보수 · **테스트** 를 **각 발주의 요건 칸으로 명시**한다 (= 필수 요건 5칸 · 본문 SoT = [`disk-verification` skill](../../.claude/skills/disk-verification/SKILL.md) §4 ⑥).
...
| 3 구글식 기준선 | **필수 요건 5칸**(의존성 · 테스트 · 문서·주석 · 유지보수 부채 · 회귀 그물) = 발주서 필수 절 | 5칸 중 **1칸이라도 부재 = 발주 미완**(= 저작 층 게이트 · 발주서 반려) |
