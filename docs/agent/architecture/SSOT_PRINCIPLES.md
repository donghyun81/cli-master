...
> 운영 레이어 vs 제품 레이어 경계 정의(경로 · 변경 권한 · 앱-중립 침투 금지)의 SoT = [`COMMON_ARCHITECTURE.md §1`](./COMMON_ARCHITECTURE.md). 본 §은 가리키기만 한다(중복 금지). 아래 = SSOT 관점의 "사실 → 단일 출처" 매핑(본 doc 고유 framing · 보존).
