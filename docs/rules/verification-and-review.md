...
- **부재 판정** = 전수 트리에서만 (= [`code-principles.md`](./code-principles.md) §2). subset 위의 「없다」는 **무효**다.
  - ★**위반 실증 — 규약이 없어서가 아니라 있는데 안 지켜서 났다** (= 2026-08-30 `MASTER-DOC-MANIFEST-SWEEP-003` · ㉡ #197). 어느 발주가 `disk-verification` skill 을 「이력 절 **부재**」로 분류했다. 자 = `grep -c '명시 cycle 이력' <skill>/SKILL.md` = **0**. **실물은 `<skill>/references/adjacent-and-history.md` 에 실재**(hit 1) ⟹ 판정이 뒤집힌다. ★**틀린 것은 값이 아니라 분모다** — `SKILL.md` **1 file** 은 그 skill 트리의 **subset** 이고, 위 줄이 이미 「**subset 위의 「없다」는 무효**」라 못 박았다. ⟹ **규약이 없어서 난 사고가 아니라, 있는 규약을 그대로 어긴 사고**다. **skill 의 단위는 `SKILL.md` + `references/` 트리** 전체다.
  - ★**단위가 트리인 대상을 file 하나로 재지 않는다.** 해당 계열 = **skill**(`SKILL.md` + `references/` + `assets/`) · **plugin**(manifest + 하위 전량) · **module**(`build.gradle.kts` + `src/` 전량) · **agent**(정의 file + 참조 rule) — 이들은 **진입 file 이 따로 있어서** 그 file 만 열면 「다 봤다」는 착각이 특히 쉽다. **분모는 진입 file 이 아니라 그 대상이 소유한 트리 전체**다.
  - ★**본 항이 신설이 아니라 실증 부착인 이유**: 위 「전수 트리에서만」은 **이미 있었다**. 같은 축을 다른 절에 새로 쓰면 **한 규칙이 두 곳에 살고 곧 갈린다** — **있는 규칙 옆에 그 규칙을 어긴 실측을 박는 것**이 처방이다(= 규칙 부재와 규칙 위반은 **다른 병**이고 처방도 다르다).
...
- ★**자의 「깊이」 — 패턴이 대상의 깊이 범위를 덮는가** (= 2026-08-30 `MASTER-DOC-MANIFEST-SWEEP-003` · ㉡ #196). 위 「범위」가 **어느 트리를 보는가**라면, 본 항은 **그 안에서 몇 층까지 보는가**다. ★**계수 자는 범위를 좁히는 것으로 조용히 0 을 낸다** — 0 은 「없다」와 「그 깊이를 안 봤다」를 **같은 출력**으로 낸다(= 위 「깨끗함과 안 쟀음이 같은 출력」의 **깊이 축 instance**).
  - **실측** (2026-08-30 · `08104e9` · `.claude/rules/pencil-mcp-tools-reference.md` · 단위 = `+` 헤딩 행) = 깊이별 `#` **0** · `##` **0** · `###` **0** · `####` **6** · `#####` **0** ⟹ 같은 diff 를 `^+#\{1,3\} ` 로 재면 **0**(거짓) · `^+#\{1,\} ` 로 재면 **6**(참). ★**그 commit 은 67 행을 추가했는데 헤더 자는 0 을 냈다** — 판정이 「절 개정 아님」으로 뒤집힌다.
  - ★**처방** ⑴ 헤딩 · 들여쓰기 · 중첩 · 디렉터리 계층처럼 **깊이가 있는 대상**에는 **상한 없는 수량자**(`{1,}` · `+`)를 **기본값**으로 쓴다. ⑵ ★**제한이 틀렸다는 말이 아니다** — 깊이를 제한해야 옳은 명제도 있다(예: 목차 대상 = `#`·`##` 한정 · 최상위 절 census = `##` 한정). **제한 자체가 아니라 근거 없는 제한이 병이다.** ⑶ ⟹ **제한하려면 「왜 그 깊이까지가 명제의 범위와 같은가」를 자 옆 같은 줄에 적는다.** 못 적으면 그 제한은 **판정이 아니라 습관**이다.
  - ★**양성대조** = 깊이를 한 칸 넓혀 재본다(`{1,3}` → `{1,}`). **값이 움직이면 원래 자가 덮지 못한 층이 있었다는 뜻**이고, 그 층이 명제 안이면 자를 고친다 (= §0.6 양성대조 의무 동형).
...
- ★**K-133 — 리터럴 계수는 사용처가 아니다.** `grep -c <심볼>` 은 **주석 · KDoc · 설명문 · 이력 블록**까지 문다. 「N 곳에서 쓴다」를 그 수로 말하면 틀린다 — **구조를 지시하기 전에 뿌리(정의)와 KDoc 근거를 읽는다.** 사용처를 세려면 정의 · 호출 · 문자열 언급을 **분리해서** 센다.
  - ★**같은 뿌리의 다른 축 = 존재·귀속** (= 2026-08-30 `MASTER-DOC-MANIFEST-SWEEP-004` · ㉡ #221 · 위 K-133 이 **사용** 축이라면 본 항은 **존재** 축). **「X 가 이 표/절에 있다」는 「행」 단위로 잰다.** `grep -c X <file>` 로 재면 **인용 · 주석 · 이력 note · 다른 항목이 X 를 언급한 대목**까지 문다 — 그 수는 **X 가 등재됐다**는 명제의 자가 아니다. 자는 **명제형**이어야 한다: 표면 `^| <ID> `, 이력 절이면 entry 형(`^- <날짜> . <ID>`), 절이면 **절 결속**(= §0.6).
  - **실증** = `MASTER-DOC-MANIFEST-SWEEP-003` G10. 명제 = 「본 판이 hot 에 등재 · COLD 엔 없음」(계약 `1 0`) ↔ **문자열 자 = `2 1`**(`CLAUDE.md` 데이터행 + 회차 note · COLD 회차 note) ↔ **명제형 자 = `1 0`** = 충족. 자가 갈렸을 뿐 실물은 계약대로였다.
  - ★**재현 실증**(2026-08-30 · 본 판) = `docs/rules/reporting.md` 의 `MASTER-DOC-MANIFEST-SWEEP-002`. **문자열 자 = 2**(→ 「등재됐다」로 읽힌다) ↔ **명제형 자 = 0**(= `grep -c '^- 20[0-9][0-9]-[0-9-]* . MASTER-DOC-MANIFEST-SWEEP-002'`). 그 2 hit 는 **다른 cycle 의 entry 안에서 그 판을 소급 등재 주체로 지목한 귀속 표기**였다 — **인용된 것과 등재된 것은 다르다.**
  - ★**인용을 지우는 것은 처방이 아니다** — 인용을 지우면 그 인용에 기대던 주장(예: 회차 계보)이 근거를 잃는다. **지울 것이 아니라 가를 것**이다(= `rule-footer-common.md` 「자는 귀속과 인용을 못 가른다」 정합).
...
- 2026-08-30 · `MASTER-DOC-MANIFEST-SWEEP-003` · **§0.1 「자의 깊이」 항 신설** (= ㉡ #196 · 패턴이 대상의 **깊이 범위**를 덮는가 · ★계수 자는 범위를 좁혀 **조용히 0** 을 낸다[= 「없다」와 「그 깊이를 안 봤다」가 같은 출력] · 실측 `08104e9`·`pencil-mcp-tools-reference.md` 깊이별 `+` 헤딩 = `#`0/`##`0/`###`0/`####`**6**/`#####`0 ⟹ `^+#\{1,3\} `=**0**(거짓) vs `^+#\{1,\} `=**6**(참) · **67 행 추가인데 헤더 자는 0** · 처방 = 상한 없는 수량자를 **기본값**으로 · ★**제한 자체가 아니라 근거 없는 제한이 병**[목차 = `#`·`##` 한정처럼 옳은 제한 실재] ⟹ 제한하면 **이유를 같은 줄에** · 양성대조 = 깊이 한 칸 넓혀 재기) + ★**§0.1 「부재 판정」 항에 위반 실증 부착**(= ㉡ #197 · **신설 아님** — 「subset 위의 「없다」는 무효」는 **이미 있었다** · `disk-verification` skill 을 `SKILL.md` **1 file** 로 재서 「이력 절 부재」 오판 · 실물은 `references/adjacent-and-history.md` 실재 · ★**단위가 트리인 대상**[skill · plugin · module · agent] = 진입 file 이 분모가 아니다 · ★**규칙 부재와 규칙 위반은 다른 병이고 처방도 다르다** — 같은 축을 새 절로 쓰면 한 규칙이 두 곳에 살고 곧 갈린다). 자 = 본 판 게이트 G4(`0 1` → `4 2`) · §0.2~§0.7 **무접촉**.
- 2026-08-30 · `MASTER-DOC-MANIFEST-SWEEP-004` · **§0.3 K-133 하위 「존재·귀속」 축 부착** (= ㉡ #221 · **신설 아님** — K-133 「리터럴 계수는 **사용처**가 아니다」는 이미 있었고 빠진 것은 **같은 뿌리의 다른 축**[사용 ↔ 존재]이다 · 새 K 번호를 파면 한 뿌리가 두 곳에 산다). 내용 = 「X 가 이 표/절에 **있다**」는 **행 단위**로 잰다 · 자는 **명제형**(`^| <ID> ` / entry 형 / 절 결속 §0.6) · 실증 2 = ⑴ SWEEP-003 G10 문자열 자 `2 1` ↔ 명제형 자 `1 0`(계약 충족) ⑵ ★**본 판 재현** = `reporting.md` 의 `MASTER-DOC-MANIFEST-SWEEP-002` 문자열 자 **2** ↔ 명제형 자 **0** — 그 2 hit 는 **다른 entry 안의 귀속 표기**였다(= 인용된 것 ≠ 등재된 것) · ★처방은 **인용 삭제가 아니라 자 분리**(인용을 지우면 회차 계보 주장이 근거를 잃는다). 자 = 본 판 게이트 G3(`0 13` → 실측) · §0.1~§0.2 · §0.4~§0.7 **무접촉**.
