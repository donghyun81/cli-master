...
> **자동 주입층 경계 (T1 · MASTER-CLI-CONTEXT-DIET-2-003 · T-3 갱신)**: `.claude/rules/` 잔존 **6** (= `safety-and-secrets` + `anchor-list` + `cross-repo-parallel-exec`(kernel) + `stop-canonical`(2026-07-29 신설) + `rule-routing-table`(본 표) + `rule-footer-common` · 실측 자 = `ls -1 .claude/rules/*.md | wc -l`) = 세션 시작 **자동 주입 · Read 불요 · compact 생존** (drift 대조 = SessionStart hook 주입값). 그 외 **42** rule (= 실측 자 `ls -1 docs/rules/*.md | wc -l` 기준 · 2026-08-23 `MASTER-AIDOC-RELEASE-REALIGN-001` 재계수 · 구 판 44 = 화면명 매핑표 은퇴[→ master `.auto-memory/` COLD] 전 시점에도 이미 1 초과) = `docs/rules/` 로 이전 → **자동 주입 X · Reading Mode Read 가 유일 로드 경로 · compact 후 재주입 없음** (필요 시 재Read). = 자동 주입층 ↔ Reading Mode Read 이중 적재 해소 (진입 base tok 감축).
...
| 2. UI-UX형 | L1: workflow-core · cycle-discipline · verification-and-review · reporting / L2: code-style-guide · design-to-code-sync · design-prompting-paradigm · code-principles · 테스트 시 TESTING_STRATEGY / L3: ui-ux-analysis · ux-laws · uiux-sot-refresh(보호) + Pencil 시 pencil-uiux-workflow(보호) 및 Pencil cluster |
